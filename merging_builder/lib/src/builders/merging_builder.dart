...
class MergingBuilder<T, S extends SyntheticInput> extends SyntheticBuilder<S> {
  ...
  ///
  /// Libraries are pre-loaded in a first pass to fully populate the resolver
  /// before generation begins. This ensures resolver.libraries is complete
  /// from the first generateStream call, enabling effective caching.
  Stream<T> _combineStreams(
    BuildStep buildStep,
    Iterable<AssetId> libraryAssetIds,
  ) async* {
    // Pre-load all libraries to fully populate the resolver before generation.
    // This separates the loading phase from the generation phase, so generators
    // can cache resolver.libraries on the first call without worrying about
    // the library set growing during iteration.
    final preloadedLibraries = <AssetId, LibraryReader>{};
    for (final libraryAssetId in libraryAssetIds) {
      preloadedLibraries[libraryAssetId] = LibraryReader(
        await buildStep.resolver.libraryFor(libraryAssetId),
      );
    }

    // Generation pass: iterate over pre-loaded libraries.
    for (final libraryAssetId in libraryAssetIds) {
      final library = preloadedLibraries[libraryAssetId]!;
